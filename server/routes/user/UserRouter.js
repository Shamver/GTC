...
const GET_USER_PROFILE = `
  SELECT GTC_USER.ID AS userId
    ...
    , GTC_USER.GT_NICKNAME AS GTName
...
const GET_USER_POST_LIST = `
  ...
  END AS postCreated
    ...
    , (SELECT CEIL(COUNT(*)/5) FROM GTC_POST WHERE GTC_POST.USER_ID = :USER_ID AND GTC_POST.DELETE_FL = 0) AS rowCount
  FROM GTC_POST A
  WHERE A.USER_ID = :USER_ID AND A.DELETE_FL = 0
  ORDER BY ID DESC
  LIMIT :INDEX, 5
`;
...
const GET_USER_COMMENT_LIST = `
  ...
  END AS commentCreated
    , (SELECT CEIL(COUNT(*)/5) FROM GTC_COMMENT WHERE GTC_COMMENT.USER_ID = :USER_ID AND GTC_COMMENT.DELETE_FL = 0) AS rowCount
  FROM GTC_COMMENT B
  WHERE B.USER_ID = :USER_ID AND B.DELETE_FL = 0
  ORDER BY ID DESC
  LIMIT :INDEX, 5
`;
