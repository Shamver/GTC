...
const UPDATE_COMMENT_AFTER_INSERT = `
  UPDATE GTC_COMMENT
    SET
    COMMENT_ID = (SELECT IFNULL(COMMENT_ID, (SELECT LAST_INSERT_ID() AS ID)) FROM (SELECT * FROM GTC_COMMENT WHERE ID = (SELECT LAST_INSERT_ID() AS ID)) CI)
    , COMMENT_ID_UPPER = (SELECT IFNULL(COMMENT_ID_UPPER, (SELECT LAST_INSERT_ID() AS ID)) FROM (SELECT * FROM GTC_COMMENT WHERE ID = (SELECT LAST_INSERT_ID() AS ID)) CIU)
  WHERE ID = (SELECT LAST_INSERT_ID() AS ID)
`;
...
SELECT
    ...
    , A.POST_ID AS bpId
    , (SELECT U.ADMIN_FL FROM GTC_USER U WHERE U.ID = A.USER_ID) AS isWriterAdmin
    , (SELECT U.OPERATOR_FL FROM GTC_USER U WHERE U.ID = A.USER_ID) AS isWriterOperator
    FROM GTC_COMMENT A, GTC_POST C
  WHERE
    A.POST_ID = :POST_ID
    AND A.DELETE_FL = 0
    AND C.ID = A.POST_ID
    AND A.USER_ID NOT IN (SELECT USER_ID_TARGET FROM GTC_USER_IGNORE WHERE USER_ID = :USER_ID)
...
const SELECT_MY_POST = `
  ...
  WHERE GBR.USER_ID = :USER_ID AND GBR.DELETE_FL = 0
