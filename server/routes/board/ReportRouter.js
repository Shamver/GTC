...
const SELECT_ALL_REPORT = `
 SELECT R.ID AS reportId
    ...
    , CASE
        WHEN R.TYPE_CD = 'P01' THEN (SELECT C.NICKNAME FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_POST WHERE ID = R.TARGET_ID))
        WHEN R.TYPE_CD = 'P02' THEN (SELECT C.NICKNAME FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID))
        ...
        AS targetName
    , CASE
        WHEN R.TYPE_CD = 'P01' THEN (SELECT C.ID FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_POST WHERE ID = R.TARGET_ID))
        WHEN R.TYPE_CD = 'P02' THEN (SELECT C.ID FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID))
  ...
  WHERE
    CASE
      WHEN ':TAB' = 'reportTable' THEN
        ( R.REJECT_FL = 0 AND
        CASE
          WHEN R.TYPE_CD = 'P01' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_POST WHERE ID = R.TARGET_ID))
          WHEN R.TYPE_CD = 'P02' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID)) END = 0)
      WHEN ':TAB' = 'reportResult' THEN
        ( R.REJECT_FL = 1 OR
        CASE
          WHEN R.TYPE_CD = 'P01' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_POST WHERE ID = R.TARGET_ID))
          WHEN R.TYPE_CD = 'P02' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID)) END = 1)
    END
  ORDER BY
    CASE
...
const SELECT_DEATAIL_REPORT = `
 SELECT R.ID AS reportId
    ...
    , R.TARGET_ID AS targetContentsId
    , CASE
        WHEN R.TYPE_CD = 'P01' THEN (SELECT P.TITLE FROM GTC_POST P WHERE P.ID = R.TARGET_ID)
        WHEN R.TYPE_CD = 'P02' THEN (SELECT C.CONTENT FROM GTC_COMMENT C WHERE C.COMMENT_ID = R.TARGET_ID) END
        AS targetContents
    , CASE
        WHEN R.TYPE_CD = 'P01' THEN R.TARGET_ID
        WHEN R.TYPE_CD = 'P02' THEN (SELECT C.POST_ID FROM GTC_COMMENT C WHERE C.ID = R.TARGET_ID) END
