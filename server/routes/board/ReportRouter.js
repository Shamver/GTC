...
const SELECT_ALL_REPORT = `
 SELECT R.ID AS reportId
    ...
    , CASE
        ...
        WHEN R.TYPE_CD = 'RT02' THEN (SELECT C.NICKNAME FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID))
    ...
    , CASE
        ...
        WHEN R.TYPE_CD = 'RT02' THEN (SELECT C.ID FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID))
    ...
    CASE
      WHEN ':TAB' = 'reportTable' THEN
        ...
        CASE
          ...
          WHEN R.TYPE_CD = 'RT02' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID))
      ...
      WHEN ':TAB' = 'reportResult' THEN
        ...
        CASE
          WHEN R.TYPE_CD = 'RT01' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_POST WHERE ID = R.TARGET_ID))
          WHEN R.TYPE_CD = 'RT02' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID)) 
          WHEN R.TYPE_CD = 'RT03' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = R.TARGET_ID) END = 1)
    END
  ORDER BY
    CASE
...
const SELECT_DEATAIL_REPORT = `
 SELECT R.ID AS reportId
    ...
    , CASE
        ...
        WHEN R.TYPE_CD = 'RT02' THEN (SELECT C.CONTENT FROM GTC_COMMENT C WHERE C.COMMENT_ID = R.TARGET_ID) END
        AS targetContents
    , CASE
