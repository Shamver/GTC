...
const SELECT_ALL_REPORT = `
 SELECT R.ID AS reportId
    ...
    , CASE
        WHEN (SELECT B.SUSPEND_BAN_FL OR B.TEMP_BAN_FL FROM GTC_USER_BAN B WHERE B.USER_ID = R.TARGET_ID) THEN '밴'
  ...
  WHERE
    CASE
      WHEN ':TAB' = 'reportTable' THEN
        ( R.REJECT_FL = 0 AND
        CASE
          WHEN R.TYPE_CD = 'RT01' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_POST WHERE ID = R.TARGET_ID))
          WHEN R.TYPE_CD = 'RT02' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE ID = R.TARGET_ID))
          WHEN R.TYPE_CD = 'RT03' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = R.TARGET_ID) END = 0)
      WHEN ':TAB' = 'reportResult' THEN
        ( R.REJECT_FL = 1 OR
        CASE
          WHEN R.TYPE_CD = 'RT01' THEN (SELECT B.SUSPEND_BAN_FL OR B.TEMP_BAN_FL FROM GTC_USER_BAN B WHERE B.USER_ID = (SELECT USER_ID FROM GTC_POST WHERE ID = R.TARGET_ID))
          WHEN R.TYPE_CD = 'RT02' THEN (SELECT B.SUSPEND_BAN_FL OR B.TEMP_BAN_FL FROM GTC_USER_BAN B WHERE B.USER_ID = (SELECT USER_ID FROM GTC_COMMENT WHERE ID = R.TARGET_ID)) 
          WHEN R.TYPE_CD = 'RT03' THEN (SELECT B.SUSPEND_BAN_FL OR B.TEMP_BAN_FL FROM GTC_USER_BAN B WHERE B.USER_ID = R.TARGET_ID) END = 1)
    END
  ORDER BY
    CASE
