    ...
    CASE
      WHEN ':TAB' = 'reportTable' THEN
        ...
        CASE
          ...
          WHEN R.TYPE_CD = 'RT02' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID)) END = 0)
      WHEN ':TAB' = 'reportResult' THEN
        ( R.REJECT_FL = 1 OR
        CASE
          WHEN R.TYPE_CD = 'RT01' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_POST WHERE ID = R.TARGET_ID))
          WHEN R.TYPE_CD = 'RT02' THEN (SELECT C.BANNED_FL FROM GTC_USER C WHERE C.ID = (SELECT USER_ID FROM GTC_COMMENT WHERE COMMENT_ID = R.TARGET_ID)) END = 1)
    END
  ORDER BY
    CASE
